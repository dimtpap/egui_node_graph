    ...
    DataType: DataTypeTrait<UserState>,
{
    ...
    fn show_graph_node(
        ...
        ui.new_child(UiBuilder::new().max_rect(inner_rect));

        let mut child_ui = ui.new_child(UiBuilder::new().max_rect(inner_rect));
            ...
            NodeData: NodeDataTrait,
        {
                ...
                (0..inner_ports)
                    .map(|k| {
                        port_rect.center_top()
                            + (Vec2::new(0.0, 5.0) + Vec2::new(0.0, 10.0) * k as f32)
                                * pan_zoom.zoom
                    })
                    .collect(),
            );
            ...
            let close_enough = if let Some(pointer_pos) = ui.ctx().pointer_hover_pos() {
                port_rect
                    .expand((DISTANCE_TO_CONNECT * pan_zoom.zoom) / 2.0)
        ...
        let (shape, outline) = {
            ...
            let outline = if self.selected {
                Shape::Rect(RectShape::filled(
                    node_rect.expand(2.0 * pan_zoom.zoom),
                    rounding,
